...
def calcular_pesos_ahp(matriz_comparacao):
    ...
    Blindado contra entradas degeneradas:
      - matriz vazia (n == 0) gera ValueError com mensagem clara;
      - colunas que somam zero não causam divisão por zero;
      - para n < 3 a consistência (CR) não se aplica e retorna 0.0 / consistente.
    """
    matriz_comparacao = np.asarray(matriz_comparacao, dtype=float)
    n = matriz_comparacao.shape[0]

    if n == 0:
        raise ValueError(
            "Nenhum critério ativo para o cálculo AHP. "
            "Selecione pelo menos 2 atributos (sem marcar todos como 'Ignorar')."
        )

    # 1. Normalização e Pesos (evita divisão por zero em colunas nulas)
    soma_colunas = matriz_comparacao.sum(axis=0)
    soma_colunas = np.where(soma_colunas == 0, 1e-9, soma_colunas)
    matriz_normalizada = matriz_comparacao / soma_colunas
    pesos = matriz_normalizada.mean(axis=1)

    # Para 1 ou 2 critérios a Razão de Consistência não é definida (RI = 0).
    if n < 3:
        return pesos, 0.0, True

    # 2. Cálculo da Consistência (protege contra pesos zerados)
    pesos_seguro = np.where(pesos == 0, 1e-9, pesos)
    vetor_ponderado = np.dot(matriz_comparacao, pesos)
    lambda_max = (vetor_ponderado / pesos_seguro).mean()
